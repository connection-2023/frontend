...
const inputStyle =
  'w-36 h-8 rounded-md px-3 py-2 outline outline-1 outline-gray-500';
...
const UserInfoSetup = ({ userInfo, handleUserInfo }: Props) => {
  ...
  return (
    ...
    <section className="flex h-full w-full flex-col justify-between">
      <div className="flex h-20 flex-col items-center justify-center gap-2 border-b border-solid border-gray-700 text-sm text-gray-100">
        <h2>로그인 정보</h2>
      ...
      <div className="mb-5 font-semibold">
        ...
        <ul className="mb-4 flex flex-col gap-[0.62rem] border-b border-dashed border-gray-700 px-[1.31rem] pb-5 text-sm">
        ...
        <ul className="text-noraml flex flex-col gap-2 gap-[0.44rem] px-[1.31rem] text-sm text-gray-100">
...
const SocialIcons = (socialType: SignInResponse['signUpType']) => {
  if (socialType === 'NAVER') {
    return (
      <button className="flex h-[21px] w-[21px] items-center justify-center rounded-full bg-naver">
        <NaverSVG width="9" height="9" />
      </button>
    );
  } else if (socialType === 'KAKAO') {
    return (
      <button className="flex h-[21px] w-[21px] items-center justify-center rounded-full bg-kakao">
