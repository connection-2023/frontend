...
const UserConsentForm = dynamic(() => import('./Steps/UserConsentForm'));
const UserSetup = dynamic(() => import('./Steps/UserSetup'));
...
const Register = ({ token, userEmail, signUpType }: IRegisterProps) => {
  ...
  if (!token || !userEmail || !signUpType) return null;
  ...
  return (
    <section className="mx-auto flex h-[37.5rem] w-[25.5rem] flex-col rounded-md bg-white px-6 pb-6 shadow-float">
      <ul className="mb-6 mt-16 flex h-[2.2rem] w-full shrink-0 items-center justify-between whitespace-nowrap rounded-[3.13rem] text-base font-semibold shadow-float">
