...
const SchedulePage = () => {
  return (
    <section className="col-span-2 flex w-full flex-col gap-4">
      <div className="col-span-2 rounded-admin bg-white px-5 shadow-float">
        <FullCalendar />
      </div>
      <div className="mx-auto flex h-[4.25rem] w-full max-w-[60.67rem] items-center justify-between whitespace-nowrap rounded-admin bg-white px-5 text-sub-color3 shadow-float">
...
const ListItem = ({ text, count, color }: IListItem) => (
  <li className={color}>
    {text}
    <span className="ml-2 text-sub-color3">{count}회</span>
  </li>
);
